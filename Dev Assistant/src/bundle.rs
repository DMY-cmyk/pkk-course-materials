...
use crate::builder::{copy_assets, make_week_summaries};
...
pub fn build_master_bundle(assets_dir: &str, output_root: &str) {
    let css_dir = Path::new(assets_dir).join("css");
    let base_css   = fs::read_to_string(css_dir.join("base.css")).unwrap_or_default();
    let screen_css = fs::read_to_string(css_dir.join("screen.css")).unwrap_or_default();
    let print_css  = fs::read_to_string(css_dir.join("print.css")).unwrap_or_default();
...
fn extract_body(html: &str) -> String {
    if let (Some(start), Some(end)) = (html.find("<body"), html.rfind("</body>")) {
        if let Some(gt_offset) = html[start..].find('>') {
            return html[(start + gt_offset + 1)..end].to_string();
        }
    }
    html.to_string()
}
